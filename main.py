...
#         vectors = [model[word] for word in phrase if (len(word) > 2) and (word in list(model.key_to_index.keys()))]
#         if len(vectors) == 0:
...
#     for phrase in data:
#         words = phrase.split()
#         vectors = [model.wv[word] for word in words if len(word) > 2 and word in model.wv]
#         if len(vectors) == 0:
#             result = [0.0] * model.vector_size
#         else:
